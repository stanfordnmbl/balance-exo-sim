...
sink("whole_body_angular_momentum_statistics.txt", append=FALSE, split=TRUE)
...
for (actuator in actuators) {
    for (time in times) {
        for (direc in direcs) {
            filepath <- paste0("tables/wbam_stats_time", time, "_", direc, "_", actuator, ".csv")
            result_name <- paste0("wbam_stats_time", time, "_", direc, "_", actuator)
            ...
            # Run the stats.
            perform_stats_for_formula(exp, wbam ~ perturbation + (1 | subject), 
...
for (time in times) {
    for (direc in direcs) {
        filepath <- paste0("tables/wbam_stats_time", time, "_", direc, "_diff.csv")
        result_name <- paste0("wbam_stats_time", time, "_", direc, "_diff")
        ...
        # Run the stats.
        perform_stats_for_formula(exp, wbam ~ actuator + perturbation + (1 | subject), 
