...
def add_to_study(study):
    ...
    subject.add_task(tasks.TaskAdjustScaledModelMarkers, marker_adjustments,
        treadmill=study.treadmill)
    if study.treadmill:
        subject.add_task(tasks.TaskAddTreadmillPrescribedMotion)
    ...
    # perturb05 condition (right-backwards perturbation)
    # --------------------------------------------------
    perturb05 = subject.add_condition('perturb05')
    perturb_time = 9.94
    start_time = 9.87
    end_time = 10.69
    # Perturbation occurs at 0% of a left foot gait cycle
    percent_gait_cycle = \
        1.0 - ((perturb_time - start_time) / gait_cycle_duration)

    interval = dict()
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb05_trial = perturb05.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb05_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=14.0)
    
    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb05_trial)

    guess_fpath = os.path.join(
            perturb05_trial.results_exp_path, 'moco', 'perturb.sto')
    perturb05_trial.add_task(tasks.TaskMocoPerturbedWalking,
        ik_setup_task, id_setup_task, unperturbed_fpath,
        percent_gait_cycle, perturb_time, mesh_interval=0.02, 
        walking_speed=study.walking_speed, guess_fpath=None)

    # Weaken all muscles
    weakness_type = 'all'
    weakness_levels = [0.75, 0.50, 0.25]
    for level in weakness_levels:
        perturb05_trial.add_task(tasks.TaskMocoPerturbedWalkingWeakness,
            ik_setup_task, id_setup_task, unperturbed_fpath,
            percent_gait_cycle, perturb_time, mesh_interval=0.02, 
            walking_speed=study.walking_speed, guess_fpath=guess_fpath,
            weakness_type=weakness_type, weakness_level=level)
        guess_fpath = os.path.join(
            perturb05_trial.results_exp_path, 'weakness', weakness_type,
            f'perturb_weak_{weakness_type}_{int(100*level)}.sto')
    perturb05_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
            perturb05_trial.tasks[-1], weakness_type, weakness_levels)

    # Weaken muscle groups
    weakness_level = 0.1
    weakness_types = ['gluts', 'adductors', 'psoas', 'hamstrings', 'recfem', 
                      'vasti', 'plantarflexors']
    guess_fpath = os.path.join(
            perturb05_trial.results_exp_path, 'moco', 'perturb.sto')
    for weakness_type in weakness_types:
        perturb05_trial.add_task(tasks.TaskMocoPerturbedWalkingWeakness,
            ...
            percent_gait_cycle, perturb_time, mesh_interval=0.02, 
            walking_speed=study.walking_speed, guess_fpath=guess_fpath,
            weakness_type=weakness_type, weakness_level=weakness_level)
        perturb05_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
            perturb05_trial.tasks[-1], weakness_type, [weakness_level])

    # perturb08 condition (forward perturbation)
    # ------------------------------------------
    perturb08 = subject.add_condition('perturb08')
    perturb_time = 9.94
    start_time = 9.87
    end_time = 10.75
    # Perturbation occurs at 0% of a left foot gait cycle
    ...
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb08_trial = perturb08.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb08_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=12.0)

    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb08_trial)

    guess_fpath = os.path.join(
            perturb08_trial.results_exp_path, 'moco', 'perturb.sto')
    perturb08_trial.add_task(tasks.TaskMocoPerturbedWalking,
        ik_setup_task, id_setup_task, unperturbed_fpath,
        percent_gait_cycle, perturb_time, mesh_interval=0.02, 
        walking_speed=study.walking_speed, guess_fpath=None)
    ...
    weakness_levels = [0.75, 0.50, 0.25]
    for level in weakness_levels:
        perturb08_trial.add_task(tasks.TaskMocoPerturbedWalkingWeakness,
            ik_setup_task, id_setup_task, unperturbed_fpath,
            percent_gait_cycle, perturb_time, mesh_interval=0.02, 
            walking_speed=study.walking_speed, guess_fpath=guess_fpath,
            weakness_type=weakness_type, weakness_level=level)
        guess_fpath = os.path.join(
            perturb08_trial.results_exp_path, 'weakness', weakness_type,
            f'perturb_weak_{weakness_type}_{int(100*level)}.sto')
    perturb08_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
            perturb08_trial.tasks[-1], weakness_type, weakness_levels)
    ...
    for weakness_type in weakness_types:
        perturb08_trial.add_task(tasks.TaskMocoPerturbedWalkingWeakness,
            ...
            weakness_type=weakness_type, weakness_level=weakness_level)
        perturb08_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
            perturb08_trial.tasks[-1], weakness_type, [weakness_level])

    # perturb17 condition (left-backwards perturbation)
    # -------------------------------------------------
    perturb17 = subject.add_condition('perturb17')
    perturb_time = 9.94
    start_time = 9.87
    end_time = 10.895
    # Perturbation occurs at 0% of a left foot gait cycle
    ...
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb17_trial = perturb17.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb17_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=14.0)
    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb17_trial)

    # perturb25 condition (right-backwards perturbation)
    # --------------------------------------------------
    perturb25 = subject.add_condition('perturb25')
    perturb_time = 9.94
    start_time = 9.80
    end_time = 10.895
    # Perturbation occurs at 33% of a left foot gait cycle
    percent_gait_cycle = \
        0.333 - ((perturb_time - start_time) / gait_cycle_duration)

    interval = dict()
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb25_trial = perturb25.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb25_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=14.0)
    
    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb25_trial)


    # perturb26 condition (left perturbation)
    # ---------------------------------------
    perturb26 = subject.add_condition('perturb26')
    perturb_time = 9.94
    start_time = 9.87
    end_time = 11.16
    # Perturbation occurs at 33% of a left foot gait cycle
    ...
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb26_trial = perturb26.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb26_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=14.0)
    
    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb26_trial)
    
    guess_fpath = os.path.join(
            perturb26_trial.results_exp_path, 'moco', 'perturb.sto')
    perturb26_trial.add_task(tasks.TaskMocoPerturbedWalking,
        ik_setup_task, id_setup_task, unperturbed_fpath,
        percent_gait_cycle, perturb_time, mesh_interval=0.02, 
        walking_speed=study.walking_speed, guess_fpath=None)
    ...
    # weakness_levels = [0.75, 0.50, 0.25]
    # for level in weakness_levels:
    #     perturb26_trial.add_task(tasks.TaskMocoPerturbedWalkingWeakness,
    #         ik_setup_task, id_setup_task, unperturbed_fpath,
    #         percent_gait_cycle, perturb_time, mesh_interval=0.02, 
    #         walking_speed=study.walking_speed, guess_fpath=guess_fpath,
    #         weakness_type=weakness_type, weakness_level=level)
    #     guess_fpath = os.path.join(
    #         perturb26_trial.results_exp_path, 'weakness', weakness_type,
    #         f'perturb_weak_{weakness_type}_{int(100*level)}.sto')
    # perturb26_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
    #         perturb26_trial.tasks[-1], weakness_type, weakness_levels)

    # # Weaken muscle groups
    # weakness_level = 0.1
    # weakness_types = ['gluts', 'psoas', 'hamstrings', 'recfem', 'vasti', 
    #     'plantarflexors']
    # guess_fpath = os.path.join(
    #         perturb26_trial.results_exp_path, 'moco', 'perturb.sto')
    # for weakness_type in weakness_types:
    ...
    #         weakness_type=weakness_type, weakness_level=weakness_level)
    #     perturb26_trial.add_task(tasks.TaskMocoPerturbedWalkingWeaknessPost,
    #         perturb26_trial.tasks[-1], weakness_type, [weakness_level])


    # perturb28 condition (left perturbation)
    # ---------------------------------------
    perturb28 = subject.add_condition('perturb28')
    perturb_time = 9.95
    start_time = 9.80
    end_time = 11.40
    # Perturbation occurs at 66% of a left foot gait cycle
    percent_gait_cycle = \
        0.666 - ((perturb_time - start_time) / gait_cycle_duration)

    interval = dict()
    interval['start_time'] = start_time
    interval['end_time'] = end_time
    perturb28_trial = perturb28.add_trial(1,
            interval=interval,
            omit_trial_dir=True,
            )
    perturb28_trial.add_task(osp.TaskGRFGaitLandmarks,
        min_time=8.0, max_time=14.0)
    
    ik_setup_task, id_setup_task = helpers.generate_main_tasks(perturb28_trial)

    perturb28_trial.add_task(tasks.TaskMocoPerturbedWalking,
        ik_setup_task, id_setup_task, unperturbed_fpath,
        percent_gait_cycle, perturb_time, mesh_interval=0.02, 
        walking_speed=study.walking_speed, guess_fpath=None)
